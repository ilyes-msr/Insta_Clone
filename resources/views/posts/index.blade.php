            ...
            <li class="flex flex-row my-5 text-sm justify-items-center">
              <div class="mr-5">
                <a href="/{{$suggested_user->username}}">
                  <img src="{{$suggested_user->image}}" alt="" class="rounded-full h-9 w-9 border-gray-300">
                </a>
              </div>
...
{{--                <a href="/{{$suggested_user->username}}/follow" class="text-blue-500 font-bold">{{__('Follow')}}</a>--}}
                <livewire:follow-button :userId="$suggested_user->id" classes="bg-blue-500 text-white" />
              @endif
            </li>
          @endforeach
        </ul>
